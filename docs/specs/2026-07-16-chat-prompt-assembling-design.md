...
Worldbook Scanner 的兼容黄金样本由固定 SillyTavern commit 的实际 `checkWorldInfo` 生成，不手写预期输出。生成器位于 `apps/web/scripts/generate-worldbook-scanner-golden.ts`，会验证传入的 SillyTavern checkout、启动隔离服务、固定 OpenAI `cl100k_base` tokenizer、执行扫描并写入 `apps/api/test/fixtures/silly-tavern/worldbook-scanner-golden.json`。

首次准备固定基线：

```powershell
git -C D:\programming\SillyTavern worktree add --detach `
  D:\programming\SillyTavern\.worktrees\rolesta-worldbook-oracle `
  7ffb28f753b98759bc7f3ac780e2743120023657
npm --prefix D:\programming\SillyTavern\.worktrees\rolesta-worldbook-oracle ci --ignore-scripts
```

重新生成样本：

```powershell
$env:SILLY_TAVERN_DIR = 'D:\programming\SillyTavern\.worktrees\rolesta-worldbook-oracle'
pnpm --filter @rolesta/web worldbooks:generate-golden
```

固定样本覆盖关键词和选择逻辑、激活装饰器、递归、概率、分组竞争、Token 预算、上下文来源、核心注入位置和顺序，以及 sticky、cooldown、delay 的跨轮状态。API 的 golden spec 将相同输入交给 Rolesta Scanner，并比较激活条目和规范化注入结果。Assembly Trace 与 `nextRuntimeState` 中超出 SillyTavern 公开结果的 Rolesta 契约继续由相邻单元测试验证。
