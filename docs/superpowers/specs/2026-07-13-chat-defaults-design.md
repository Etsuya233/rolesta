...
Rolesta 后续创建一对一聊天时，需要让用户快速进入可用状态。用户可以分别指定默认 Persona、默认预设和默认连接信息；创建聊天页自动选中这些默认项。Persona 表示用户在聊天中扮演的身份角色卡，连接信息表示一条完整的 Model Provider 配置，其中包含服务地址、凭据和默认模型。

本设计同时补全预设与连接信息的关联编辑能力。创建聊天页会使用该关联决定连接信息，并在候选列表中突出展示默认项和当前预设关联项。
...
- 创建聊天时的默认选中顺序、预设关联联动和候选项分组规则。
- 默认资源删除、失去访问权限或关联 Provider 删除后的数据清理。

本次不包含聊天消息生成、上下文组装和消息持久化。创建聊天页尚未实现的部分，在后续聊天功能中依据本文约定接入。
...
聊天默认配置归入 `chats` 模块，作为创建聊天所需的用户偏好。该模块负责：

- 读取当前用户的三类默认配置。
- 校验角色卡和预设对当前用户可见。
- 校验 Model Provider 由当前用户拥有。
- 设置、替换或清除单类默认项。
- 返回默认资源的摘要，供管理页和创建聊天页共享。
...
摘要至少包含资源 ID、名称、所有者 ID，以及选择器展示所需的头像或 Provider 类型。预设摘要还包含已保存的 `modelProviderId`，创建聊天页据此完成关联连接初始化。

用户尚未产生聊天默认配置记录时，三个摘要和 `updatedAtMs` 均返回 `null`。
...
`modelProviderId` 属于预设表单数据，随保存操作提交。用户修改关联后点击“设置为默认值”不会保存表单；创建聊天页继续使用预设最近一次保存的关联。

他人的公开预设保持只读，展示其已保存的关联状态，不允许修改关联。即使该预设关联了其所有者的 Provider，当前用户也无法访问该连接信息，因此创建聊天时只在关联 Provider 对当前用户可用的情况下应用关联。按当前规则，预设可保存的 Provider 由预设所有者拥有；他人选择公开预设时通常无法使用该关联，连接信息继续由自己的默认配置或手动选择决定。

## 创建聊天初始化

创建聊天页加载候选资源和聊天默认配置后，按固定顺序初始化：

1. 选中默认 Persona。
2. 选中默认预设。
3. 默认预设关联了当前用户可用的 Provider 时，选中该 Provider。
4. 连接信息仍为空时，选中用户默认 Provider。

该顺序保证预设关联连接优先于用户默认连接。任何默认项缺失或已经不可访问时，对应选择保持为空，不依据最近使用记录自动选择。

用户手动选择预设时：

- 当前尚未选择连接信息，且预设关联 Provider 可用时，自动选中关联 Provider。
- 当前已有连接信息时保持原选择。
- 切换或清除预设不主动清除已选连接信息。

## 选择器排序与分组

Persona、预设和连接信息选择器都需要突出默认项：

- 默认项置于候选内容顶部。
- 默认项使用“默认”分组或等价视觉标记。
- 其余候选项进入常规分组，并保持各管理模块已有的排序规则。

Persona 可以使用角色卡网格、弹窗或其他适合头像浏览的控件，无需强制使用 Select，但必须保留置顶和默认标记。

连接信息选择器还包含“当前预设关联”分组：

- 当前预设关联的可用 Provider 位于最顶部。
- 用户默认 Provider 位于随后的“默认”分组。
- 两个分组指向同一 Provider 时只展示一次，并归入“当前预设关联”。
- 分组只影响展示顺序，不会覆盖用户已经选择的连接信息。
...
设置默认值时若资源不存在或当前用户无权访问，接口返回资源不可用错误。预设关联的 Provider 不属于当前用户时，保存请求返回明确的权限错误。前端不得根据错误自动改选其他资源。
...
- 默认 Persona 和默认预设正确初始化。
- 默认预设关联的可用 Provider 优先于用户默认 Provider。
- 预设无可用关联时采用用户默认 Provider。
- 手动切换预设不会覆盖已有连接选择。
- 连接为空时切换预设会采用其可用关联。
- 默认分组和当前预设关联分组置顶、去重并保持正确标记。
- 默认资源失效时保持对应选择为空。
...
- 创建聊天时按照本文顺序自动选择 Persona、预设和连接信息。
- 各选择器将默认项及当前预设关联项置顶展示，并避免重复候选项。
- 删除或失去访问权限的资源不会继续作为可用默认项，也不会触发自动补选。
