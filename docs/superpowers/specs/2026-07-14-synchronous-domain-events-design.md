...
- 由 Files 公开的 Application 契约接管角色头像所必需的文件状态转换。
- 领域事件只承载允许零消费者的跨领域业务反应。
...
发布方操作成功所必需的跨模块协作通过被调用模块公开的 Application 契约显式完成。允许独立扩展、允许零消费者的后续反应使用领域事件。

Files 模块是通用文件能力的所有者。它通过 `files/contracts` 发布 `FileResourceLifecycle`，由 `FilesModule` 导出对应 Nest token。Characters 保留自己的 `CharacterAvatarService` Port，文件适配器把角色头像语义转换为 Files 契约。Characters 的 Domain、Application 和 persistence 均不依赖 Files 的 Store 或数据库表。

该边界让文件状态转换保持显式且可参与当前事务。缺少领域事件 Listener 时，角色头像的激活和回收仍能正常完成。
...
```ts
export type FilePurpose = 'character-avatar' | 'user-avatar';
...
生命周期服务校验资源 ID、所有者、用途和当前状态。状态不满足要求时抛出 Files Application 错误，由调用模块的适配器翻译成自身稳定的 Port 错误。该服务不创建事务，通过 `KyselyDatabaseContext` 加入调用方现有事务。
...
发布用例可以按业务需要将 `DomainEventHandlingError` 翻译为本模块稳定的 `ApplicationError`。头像文件生命周期冲突由 `FileCharacterAvatarService` 翻译为角色模块的头像分配冲突；头像事件的其他 Listener 失败仍可沿事件错误路径传播。资产删除事件处理失败暂时进入全局内部错误路径。Publisher 不导入角色、文件或聊天偏好模块的错误类型。
...
事务外：检查与处理图片
  ...
  -> 事务外：写入文件内容
  -> UnitOfWork：更新角色头像引用
  -> UnitOfWork：通过 CharacterAvatarService 激活新资源
  -> UnitOfWork：通过 CharacterAvatarService orphan 旧资源
  -> UnitOfWork：发布 CharacterAvatarChangedEvent
```

`KyselyCharacterAvatarAssignment` 只读取和更新 `characters`，不再查询或更新 `file_resources`。它返回更新后的角色以及旧、新资源 ID。角色头像对 `file_resources` 的存在性外键继续保留，文件资源状态、purpose 和 owner 规则由 `FileResourceLifecycle` 负责。

头像移除先把角色引用更新为 `null`，再显式将旧资源标记为 orphaned，最后发布 `CharacterAvatarChangedEvent`。角色原本没有头像时不执行文件状态转换，也不发布事件。

文件生命周期服务处理失败时，角色引用更新随事务回滚。头像上传流程在事务外创建的新文件资源保持 pending，由现有清理任务回收。头像事件没有 Listener 时不影响必要的文件状态转换。
...
每个事件携带被删除资产 ID、`ownerUserId` 和 `occurredAtMs`。
...
删除带头像的角色时，角色删除用例在当前 `UnitOfWork` 中通过 `CharacterAvatarService` 将头像资源标记为 orphaned，再发布 `CharacterDeletedEvent`。聊天偏好 Listener 清空默认 Persona。文件回收不依赖事件 Listener 装配。
...
- `FilesModule` 提供并导出 `FILE_RESOURCE_LIFECYCLE`，不依赖 Characters 事件。
...
- Characters 通过自己的 `CharacterAvatarService` Port 调用 Files 公开契约。
- Files 不导入 Characters 事件，文件生命周期服务只修改 Files 拥有的数据。
