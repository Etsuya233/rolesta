...
export function PresetListPanel({
  ...
  const query = useQuery({
    queryKey: ['presets', { q, scope, sort, direction, pageIndex, pageSize }],
    queryFn: () => listPresets({ q, scope, sort, direction, pageIndex, pageSize }),
  });
  ...
  return (
    <div className="mx-auto flex min-h-0 w-full max-w-2xl flex-1 flex-col overflow-hidden">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {query.isLoading ? (
          <p className="p-4 text-sm text-muted-foreground">{t('presets.list.loading')}</p>
        ) : null}
        {query.data?.items.length === 0 ? (
          <p className="p-4 text-sm text-muted-foreground">{t('presets.list.empty')}</p>
        ) : null}
        {query.data?.items.map((preset) => (
          <PresetListItem
            key={preset.id}
            isDefault={
              !assetDefaultsQuery.isError && preset.id === assetDefaultsQuery.data?.presetId
            }
            preset={preset}
