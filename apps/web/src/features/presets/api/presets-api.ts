...
export type PresetModelSettings = components['schemas']['PresetModelSettingsResponseDto'];
...
export async function listPresets(query: ListPresetsQuery): Promise<PresetPageResponse> {
  const result = await requestApi(openApiClient.GET('/presets', { params: { query } }));
  return result.data;
}

export async function getPreset(id: string): Promise<PresetDetailResponse> {
  const result = await requestApi(openApiClient.GET('/presets/{id}', { params: { path: { id } } }));
  return result.data;
}

export async function createPreset(values: PresetCreateValues): Promise<PresetDetailResponse> {
