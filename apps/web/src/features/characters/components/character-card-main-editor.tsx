...
export function CharacterCardMainEditor({
  ...
  const [openSections, setOpenSections] = useState<string[]>(['basic', 'content']);
  const { form, setForm, isPending, visibleError, submit } = useCharacterDraftSession({
    sessionKey,
...
function characterPromptsSummary({
  ...
  const overrides = filledCount([form.systemPrompt, form.postHistoryInstructions]);
