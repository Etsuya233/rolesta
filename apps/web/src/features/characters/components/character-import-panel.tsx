...
import { FieldError, PrimaryButton } from './character-form-fields';

export interface CharacterImportPanelProps {
  onImported: (characterId: string) => void;
}

export function CharacterImportPanel({ onImported }: CharacterImportPanelProps) {
  ...
  const visibleError = errorMessage ?? (importMutation.isError ? '导入角色卡失败' : null);
    ...
    <form className="mx-auto min-h-0 w-full max-w-2xl flex-1 overflow-y-auto" onSubmit={handleSubmit}>
      <MobileFormSection title="选择文件">
        <label
          ...
          htmlFor={fileInputId}
        >
          <FileJson aria-hidden="true" className="size-8 text-muted-foreground" />
          <span className="text-sm font-medium">{file ? file.name : '选择 JSON 或 PNG 角色卡'}</span>
          <input
      ...
      <div className="space-y-3 px-4 pb-6">
        ...
        <PrimaryButton disabled={importMutation.isPending}>确认导入</PrimaryButton>
      </div>
    </form>
  );
}
