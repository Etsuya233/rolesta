...
export type ListChatsQuery = NonNullable<operations['ChatsController_list']['parameters']['query']>;
...
export async function getChat(id: string): Promise<ChatDetail> {
  return requestApi(openApiClient.GET('/chats/{id}', { params: { path: { id } } })).then(
    (result) => result.data,
  );
}

export async function createChat(values: CreateChatValues): Promise<ChatDetail> {
  return requestApi(openApiClient.POST('/chats', { body: values })).then((result) => result.data);
}

export async function updateChat(id: string, values: UpdateChatValues): Promise<ChatDetail> {
  return requestApi(
...
export async function deleteChat(id: string): Promise<{ ok?: boolean }> {
  return requestApi(openApiClient.DELETE('/chats/{id}', { params: { path: { id } } })).then(
    (result) => result.data,
  );
}
