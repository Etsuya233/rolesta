          ...
          <ScrollArea className="min-h-0 px-4">
            <FieldGroup className="pb-4">
              ...
              <AssetPicker
                ...
                onChange={(id, name) =>
                  setState((current) => selectChatCharacter(current, id!, name))
                }
              />
              <AssetPicker
                kind="character"
...
function AssetPicker({
  ...
  const items = useMemo(() => {
    ...
    return [...current].sort((a, b) => Number(b.id === pinnedId) - Number(a.id === pinnedId));
  }, [pinnedId, query.data]);
        ...
        <PopoverTrigger asChild>
          ...
          >
            <span className="truncate">
              {selected?.name ?? value ?? t('chats.management.form.unselected')}
            </span>
            <ChevronsUpDownIcon />
          </Button>
        </PopoverTrigger>
          ...
          <ScrollArea className="max-h-56">
            <div className="flex flex-col gap-1 py-1">
              {!required ? (
                <PickerButton
                  active={value === null}
                  label={t('chats.management.form.unselected')}
                  onClick={() => {
                    onChange(null, '');
                    setOpen(false);
                  }}
              ...
              {query.isError ? (
                <span className="px-2 py-3 text-sm text-destructive">
                  {t('chats.management.loadFailed')}
                </span>
              ) : null}
            </div>
          </ScrollArea>
