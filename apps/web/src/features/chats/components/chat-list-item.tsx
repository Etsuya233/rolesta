...
export function ChatListItem({
  ...
  return (
    <div className={cn(
      ...
      <button type="button" className="flex min-w-0 flex-1 items-center gap-3 px-3 py-2 text-left" onClick={onSelect}>
        <Avatar>
        ...
        <span className="flex min-w-0 flex-1 flex-col gap-0.5">
          ...
          <time className="text-xs text-muted-foreground" dateTime={new Date(chat.updatedAtMs).toISOString()}>
            {new Intl.DateTimeFormat(i18n.language, { dateStyle: "short", timeStyle: "short" }).format(chat.updatedAtMs)}
          </time>
        </span>
      </button>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button type="button" size="icon-sm" variant="ghost" aria-label={t("chats.management.actions.menu")} className="mr-2 shrink-0">
            <MoreHorizontalIcon />
          </Button>
        </DropdownMenuTrigger>
