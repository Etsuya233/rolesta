...
export function ChatDeleteDialog({ chat, open, onOpenChange, onDeleted }: {
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>{t("chats.management.delete.description", { title: chat?.title ?? "" })}</AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel>{t("common.cancel")}</AlertDialogCancel>
          <AlertDialogAction variant="destructive" disabled={mutation.isPending} onClick={(event) => {
            event.preventDefault();
            if (!chat) return;
            mutation.mutate(chat.id, { onSuccess: () => { onDeleted(chat.id); onOpenChange(false); } });
          }}>
