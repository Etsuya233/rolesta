...
export type WorldbookSummaryResponse = components['schemas']['WorldbookSummaryResponseDto'];
export type WorldbookDetailResponse = components['schemas']['WorldbookDetailResponseDto'];
export type WorldbookEntryResponse = components['schemas']['WorldbookEntryResponseDto'];
...
export async function listWorldbooks(query: ListWorldbooksQuery): Promise<WorldbookPageResponse> {
  const result = await requestApi(openApiClient.GET('/worldbooks', { params: { query } }));
  return result.data;
}
...
export async function createWorldbook(
  ...
  const result = await requestApi(openApiClient.POST('/worldbooks', { body: values }));
  return result.data;
}
...
export async function importWorldbook(file: File): Promise<WorldbookDetailResponse> {
...
export async function exportWorldbook(id: string): Promise<Blob> {
  const response = await fetchAuthed(`${API_BASE_URL}/worldbooks/${id}/export/sillytavern`);
