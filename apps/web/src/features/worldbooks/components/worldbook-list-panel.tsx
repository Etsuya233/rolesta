...
export function WorldbookListPanel({
  ...
  const query = useQuery({
    queryKey: ['worldbooks', { q, scope, sort, direction, pageIndex, pageSize }],
    queryFn: () => listWorldbooks({ q, scope, sort, direction, pageIndex, pageSize }),
  });
  useEffect(() => {
    if (query.isError) {
    ...
    <div className="mx-auto flex min-h-0 w-full max-w-2xl flex-1 flex-col overflow-hidden">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        {query.data?.items.length === 0 ? (
          <p className="p-4 text-sm text-muted-foreground">{t('worldbooks.list.empty')}</p>
        ) : null}
