...
import type { WorldbookDetailResponse, WorldbookVisibility } from '../api/worldbooks-api';
import {
  FormActionButton,
  FormSubmitButton,
...
export function WorldbookMainEditor({
  ...
  return (
    <form
      ...
      onSubmit={submit}
    >
      <div className={cn('min-h-0 flex-1 overflow-y-auto', !submitLabel && 'pb-24')}>
        <Accordion
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-2 text-sm">
              ...
              <Metric
                ...
                value={String(document.entries.filter((entry) => entry.enabled).length)}
              />
              <Metric
          ...
          >
            <div className="rounded-md border border-border px-3 py-2 text-sm">
              ...
              <span className="font-medium">{worldbook?.sourceFormat ?? 'rolesta'}</span>
            </div>
          </WorldbookFormSection>
        </Accordion>
      </div>
