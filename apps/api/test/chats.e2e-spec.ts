...
describe('Chats API', () => {
  ...
  afterEach(async () => {
    ...
    if (originalDatabasePath === undefined) delete process.env.SQLITE_DATABASE_PATH;
  ...
  it('creates, lists, updates, reads, and deletes owner-scoped Chats', async () => {
    ...
    const page = await request(app.getHttpServer() as App)
      .get('/chats?q=Shared&role=public-character&sort=title&direction=asc&pageIndex=0&pageSize=10')
      .set('Authorization', `Bearer ${owner.token}`)
      .expect(200)
...
async function getChat(app: INestApplication, token: string, id: string): Promise<ChatDetailBody> {
  return request(app.getHttpServer() as App)
    .get(`/chats/${id}`)
    .set('Authorization', `Bearer ${token}`)
    .expect(200)
