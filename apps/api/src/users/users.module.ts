...
import { USER_AVATAR_ASSIGNMENT, type UserAvatarAssignment } from './ports/user-avatar-assignment.js';
import { USER_AVATAR_READER, type UserAvatarReader } from './ports/user-avatar-reader.js';
import { USER_AVATAR_SERVICE, type UserAvatarService } from './ports/user-avatar-service.js';
...
@Module({
  ...
  providers: [
    ...
    { provide: USER_AVATAR_SERVICE, useExisting: FileUserAvatarService },
    {
      ...
      useFactory: (avatars: UserAvatarService, assignment: UserAvatarAssignment, clock: UserAvatarClock) =>
        new UploadUserAvatarUseCase(avatars, assignment, clock),
      inject: [USER_AVATAR_SERVICE, USER_AVATAR_ASSIGNMENT, FILE_CLOCK],
    },
    {
      provide: DeleteUserAvatarUseCase,
      useFactory: (assignment: UserAvatarAssignment, clock: UserAvatarClock) =>
        new DeleteUserAvatarUseCase(assignment, clock),
      inject: [USER_AVATAR_ASSIGNMENT, FILE_CLOCK],
    },
    {
      provide: GetUserAvatarUseCase,
      useFactory: (reader: UserAvatarReader, publicFiles: GetPublicFileObjectsUseCase) =>
