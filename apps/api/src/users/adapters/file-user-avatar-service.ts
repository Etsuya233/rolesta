...
@Injectable()
export class FileUserAvatarService implements UserAvatarService {
  constructor(
    private readonly images: ImageProcessor,
    private readonly createFiles: CreateFileResourceUseCase,
  ) {}
  ...
  async createAvatar(input: {
    ...
    if (input.content.byteLength > MAX_FILE_SIZE || !validCrop(input.crop)) {
      throw new UserAvatarPortError({
        reason: 'invalid-avatar',
        params: { detail: 'Invalid avatar upload.' },
      });
    }
    try {
      ...
      const created = await this.createFiles.execute({
