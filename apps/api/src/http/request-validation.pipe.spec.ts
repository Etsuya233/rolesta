...
class ZodRequestDto extends createZodDto(
  z.strictObject({ count: z.coerce.number().int() }),
) {}
...
describe("RequestValidationPipe", () => {
  ...
  it("uses Zod metadata for parsing and rejects unknown fields", async () => {
    await expect(
      pipe.transform({ count: "2" }, metadata(ZodRequestDto)),
  ...
  it("keeps legacy class-validator DTOs on the legacy path", async () => {
    const result = await pipe.transform(
      { count: 2 },
      metadata(LegacyRequestDto),
    );
    expect(result).toBeInstanceOf(LegacyRequestDto);
    await expect(
      pipe.transform({ count: "2" }, metadata(LegacyRequestDto)),
