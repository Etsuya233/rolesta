...
import { toChat, toChatRow, toChatUpdateRow, type ChatRow } from './chat-row-mapper.js';
...
export class KyselyChatStore implements ChatStore {
  ...
  async list(request: ListChatsRequest): Promise<PageResponse<ChatListItem>> {
    const countRow = await withListFilters(this.context.database.selectFrom('chats'), request)
      .select((builder) => builder.fn.countAll<number>().as('count'))
      .executeTakeFirstOrThrow();
    const rows = await withListFilters(this.context.database.selectFrom('chats'), request)
      .leftJoin('characters as chat_character', 'chat_character.id', 'chats.chat_character_id')
      .select([
        'chats.id',
        'chats.title',
        'chats.updated_at_ms',
  ...
  async findOwnedDetail(id: string, ownerUserId: string): Promise<ChatDetail | null> {
  ...
  async save(chat: Chat): Promise<ChatDetail> {
    await this.context.database.insertInto('chats').values(toChatRow(chat)).execute();
  ...
  async clearCharacterAssociations(characterId: string, exceptOwnerUserId?: string): Promise<void> {
    ...
    if (exceptOwnerUserId !== undefined) {
      chatCharacter = chatCharacter.where('owner_user_id', '!=', exceptOwnerUserId);
  ...
  async clearPresetAssociations(presetId: string, exceptOwnerUserId?: string): Promise<void> {
  ...
  private detailQuery() {
    return this.context.database
      .selectFrom('chats')
      .leftJoin('characters as chat_character', 'chat_character.id', 'chats.chat_character_id')
      .leftJoin('characters as persona', 'persona.id', 'chats.persona_character_id')
      .leftJoin('presets', 'presets.id', 'chats.preset_id')
      .leftJoin('model_provider_configs', 'model_provider_configs.id', 'chats.model_provider_id')
      .selectAll('chats')
      .select([
        'chat_character.id as character_id',
        'chat_character.name as character_name',
  ...
  private async requireOwnedDetail(id: string, ownerUserId: string): Promise<ChatDetail> {
    const row = await this.detailQuery()
      .where('chats.id', '=', id)
      .where('chats.owner_user_id', '=', ownerUserId)
      .executeTakeFirstOrThrow();
...
type ChatSelectQuery = SelectQueryBuilder<Database, 'chats', Record<string, never>>;
type ChatDetailRow = ChatRow & {
  ...
  provider_kind: 'openai-compatible' | 'openai' | 'claude' | 'z-ai' | 'deepseek' | null;
...
function withListFilters(query: ChatSelectQuery, request: ListChatsRequest): ChatSelectQuery {
...
function toChatDetail(row: ChatDetailRow): ChatDetail {
  return {
    ...
    persona: characterSummary(row.persona_id, row.persona_name, row.persona_avatar_resource_id),
    preset:
