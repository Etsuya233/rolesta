...
export class UpdateChatUseCase {
  ...
  execute(command: UpdateChatCommand): Promise<ChatDetail> {
    return this.unitOfWork.run(async () => {
      const current = await this.store.findOwnedDetail(command.id, command.ownerUserId);
      ...
      if (command.chatCharacterId !== undefined) fields.chatCharacterId = command.chatCharacterId;
      ...
      if (command.modelProviderId !== undefined) fields.modelProviderId = command.modelProviderId;

      return this.store.update(
        updateChat(current, fields, ensureEpochMillis(this.clock.now().getTime())),
      );
    });
  }
}
