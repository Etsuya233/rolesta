...
describe('model provider use cases', () => {
  it('creates a provider with a manual secret', async () => {
    ...
    const useCase = new CreateModelProviderUseCase(state.providers, ids, clock, state.keys);
  ...
  it('rejects a vault key owned by another user', async () => {
    ...
    ).rejects.toMatchObject(new ModelProviderApplicationError('api-key-not-owned', {}));
  });

  it('switches credentials without retaining the inactive value', async () => {
    const state = new MemoryState([provider({ secret: 'manual-secret' })], [apiKey()]);
    const useCase = new UpdateModelProviderUseCase(state.providers, clock, state.keys);
  ...
  it('uses the global key secret for model listing', async () => {
    ...
    await new ListModelProviderModelsUseCase(state.providers, client, state.keys).saved({
      configId: 'cfg',
      viewerUserId: 'owner',
    });
  ...
  it('uses the manual secret for connection testing', async () => {
    ...
    const client = new RecordingClient();
    await new TestModelProviderConnectionUseCase(state.providers, client, state.keys).saved({
      configId: 'cfg',
      viewerUserId: 'owner',
    });
...
class MemoryModelProviderStore implements ModelProviderStore {
  ...
  deleteOwned(id: string, ownerUserId: string): Promise<boolean> {
    const item = this.state.configs.get(id);
    return Promise.resolve(item?.ownerUserId === ownerUserId && this.state.configs.delete(id));
  }
}
...
class MemoryApiKeyStore implements ApiKeyStore {
  ...
  listOwned(ownerUserId: string): Promise<ApiKey[]> {
    return Promise.resolve(
      [...this.state.apiKeys.values()].filter((item) => item.ownerUserId === ownerUserId),
    );
  }
