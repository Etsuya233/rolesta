...
import type { ModelProviderConfig, ModelProviderSummary } from '../domain/model-provider-config.js';
import type {
...
export class KyselyModelProviderStore implements ModelProviderStore {
  ...
  async list(request: ListModelProvidersRequest): Promise<PageResponse<ModelProviderSummary>> {
    ...
    const rows = await withListFilters(
      ...
      .orderBy(`model_provider_configs.${sortColumns[request.sort]}`, request.direction)
...
type ProviderQuery = SelectQueryBuilder<Database, 'model_provider_configs', Record<string, never>>;
...
function withListFilters(query: ProviderQuery, request: ListModelProvidersRequest): ProviderQuery {
  let next = query.where('model_provider_configs.owner_user_id', '=', request.viewerUserId);
  if (request.q.trim())
    next = next.where('model_provider_configs.name', 'like', `%${request.q.trim()}%`);
  return next;
}

function toRow(config: ModelProviderConfig): Insertable<ModelProviderConfigsTable> {
  return {
    id: config.id,
...
function toSummary(row: Omit<ProviderRow, 'secret'> | ProviderRow): ModelProviderSummary {
  return {
    id: row.id,
    ...
    lastUsedAtMs: row.last_used_at_ms === null ? null : numeric(row.last_used_at_ms),
