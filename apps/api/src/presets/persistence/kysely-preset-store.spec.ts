...
describe('KyselyPresetStore', () => {
  it('keeps association changes isolated from regular aggregate updates', async () => {
    ...
    try {
      ...
      await unitOfWork.run(() => store.update({ ...original, name: 'Renamed' }));

      await expect(store.findOwnedById('preset', 'owner')).resolves.toMatchObject({
        name: 'Renamed',
  ...
  it('filters visible presets by permission scope', async () => {
    ...
    try {
      ...
      await unitOfWork.run(async () => {
        await store.save(preset({ id: 'mine', ownerUserId: 'owner', visibility: 'private' }));
        await store.save(preset({ id: 'public', ownerUserId: 'other', visibility: 'public' }));
        await store.save(preset({ id: 'hidden', ownerUserId: 'other', visibility: 'private' }));
      });
