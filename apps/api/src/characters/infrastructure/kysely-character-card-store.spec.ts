...
describe('KyselyCharacterCardStore', () => {
  it('finds private cards only for owners and public cards for other viewers', async () => {
    ...
    const store = new KyselyCharacterCardStore(database.db);

    try {
      await seedUser(database.db, 'owner');
      await seedUser(database.db, 'reader');

      await store.save(characterCard({ id: 'private_card', ownerUserId: 'owner', visibility: 'private' }));
      await store.save(characterCard({ id: 'public_card', ownerUserId: 'owner', visibility: 'public' }));

      await expect(store.findVisibleById('private_card', 'owner')).resolves.toMatchObject({ id: 'private_card' });
      await expect(store.findVisibleById('private_card', 'reader')).resolves.toBeNull();
      await expect(store.findVisibleById('public_card', 'reader')).resolves.toMatchObject({ id: 'public_card' });
  ...
  it('lists all visible cards with stable offset pagination', async () => {
    ...
    try {
      ...
      await store.save(characterCard({ id: 'b', ownerUserId: 'owner', name: 'Beta', visibility: 'private' }));
      await store.save(characterCard({ id: 'a', ownerUserId: 'other', name: 'Alpha', visibility: 'public' }));
      await store.save(characterCard({ id: 'c', ownerUserId: 'other', name: 'Gamma', visibility: 'private' }));
