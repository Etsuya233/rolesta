...
@Injectable()
export class KyselyCharacterReferenceAccess implements CharacterReferenceAccess {
  constructor(
    private readonly context: KyselyDatabaseContext,
    @Inject(APP_CONFIG) private readonly config: AppConfig,
  ) {}

  async acquireVisible(characterId: string, viewerUserId: string): Promise<boolean> {
