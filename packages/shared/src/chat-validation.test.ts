...
describe('Chat request validation', () => {
  ...
  it.each([
    [{ chatCharacterId: 'character-1' }, [{ field: 'title', rule: 'required' }]],
    [{ title: '   ', chatCharacterId: 'character-1' }, [{ field: 'title', rule: 'required' }]],
    [
  ...
  it('applies list defaults and coerces pagination values', () => {
    expect(listChatsQuerySchema.parse({ pageIndex: '2', pageSize: '50' })).toEqual({
      q: '',
      role: 'all',
      sort: 'updatedAt',
      direction: 'desc',
